...
const InternalLink = styled(Box)(({ theme }) => ({
  '& a': {
    ...ExtAndIntCommon,
    color: theme.palette.text.primary,
    '&:hover': { background: theme.palette.text.secondary, color: '#fff' },
  },
  '& .navItemActive': {
...
const MatxVerticalNav = ({ items }) => {
  ...
  items &&
    items.map((val, index) => {
      if (user.permissions.includes(val.permission)) {
        menuList.push(val);
      }
    });
        ...
        return (
          <InternalLink key={index}>
            ...
            >
              <ButtonBase key={item.name} name="child" sx={{ width: '100%' }}>
                {item?.icon ? (
                  <Icon color="secondary" className="icon" sx={{ width: 36 }}>
                    {item.icon}
                  </Icon>
                ) : (
                  <Fragment>
  ...
  return <div className="navigation">{renderLevels(menuList)}</div>;
