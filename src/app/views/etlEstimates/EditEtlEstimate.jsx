...
const EditEtlEstimate = () => {
  ...
  useEffect(() => {
    estimateData && estimateData.viewEtlEstimate && setEstimate(estimateData.viewEtlEstimate[1]);

    if (estimateData.message === 'ETL Estimate updated successfully!') {
      setShowSnackBar(true);
      setTimeout(() => {
        navigate('/etl/view-etl-estimate/' + id);
        dispatch(clearState());
      }, 1000);
  ...
  const handleRowChange = (event, taskGroupIndex, taskIndex) => {
    ...
    setEstimate((prevEstimate) => ({
      ...prevEstimate,
      etlTaskGroups: prevEstimate.etlTaskGroups.map((tkGroup, index) =>
        taskGroupIndex === index
          ? {
              ...tkGroup,
              etlTaskLists: tkGroup.etlTaskLists.map((task, tkIndex) =>
                taskIndex === tkIndex
                  ? {
                      ...task,
                      [name]: toNumber(value),
  ...
  const handleSubmit = (event) => {
    dispatch(editEtlEstimate(estimate));
  };
  ...
  const handleChangeGroupName = (event, taskGroupIndex) => {
    ...
    setEstimate({
      ...estimate,
      etlTaskGroups: estimate.etlTaskGroups.map((taskGroup, index) =>
  ...
  return (
    <Container>
      <Box className="breadcrumb">
        <Breadcrumb
          routeSegments={[
            { name: 'Edit ETL Estimate', path: '/etl/estimate-list' },
            { name: 'ETL Estimate' },
          ]}
        />
      </Box>
...
export default EditEtlEstimate;
