...
import { deleteBiEstimate } from "app/redux/bi-estimate-list/biEstimateSlice";
...
useEffect(() => {    
  ...
  return (
    ...
    <Snackbar open={showSnackBar} autoHideDuration={3000} anchorOrigin={{ vertical:'bottom', horizontal:'center' }} onClose={handleClose}>
        <Alert onClose={(ev)=>{handleClose(ev)}} severity={estimateData.isError ? "error" : "success"} sx={{ width: "100%" }} variant="filled">
          ...
          {props.estimates
            ...
            .map((estimates, index) => (
              <TableRow key={index}>
                ...
                <TableCell align="center">{new Date(estimates.createdDate).toLocaleDateString('en-IN')}</TableCell>
                <TableCell align="center">{'Active'}</TableCell>
                {/* <TableCell align="center">${estimates.amount}</TableCell> */}
                <TableCell align="right">
                  <IconButton onClick={() => navigate('/bi/view-bi-estimate/' + estimates.biEstimatorId)}>
                    <Icon color="primary">visibility</Icon>                    
                  </IconButton>

                  <IconButton onClick={() => editEstimate(estimates)}>                  
                  <Icon color="secondary">edit</Icon>                    
                  </IconButton>

                  <IconButton onClick={() => {setOpen(true);setEstimate(estimates)}}>
...
<Dialog
      ...
      >
        <DialogTitle id="responsive-dialog-title">Are you sure? Do you want to delete this Estimate?</DialogTitle>

        <DialogContent>
          <DialogContentText>
            Make sure that you will not need this item. You can not get it back. It is no longer available.
          </DialogContentText>
        </DialogContent>
